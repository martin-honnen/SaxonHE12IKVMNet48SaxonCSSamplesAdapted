...
namespace SaxonHE12IKVMNet48SaxonCSSamplesAdapted
{

    public class SaxonHE12Examples
    {
        /// <summary>
        /// Run Saxon XSLT and XQuery sample applications in Saxon HE 12 .NET framework 4.8 cross-compiled using IKVM
        /// </summary>
        /// <param name="argv">
        /// <para>Options:</para>
        /// <list>
        ...
        {
            Console.WriteLine(".NET {0} OS {1}", Environment.Version, Environment.OSVersion);
            ...
            foreach (String s in argv)
            {
                ...
                {
                    test = s.Substring(6);//s[6..];
                }
                else if (s.StartsWith("-dir:"))
                {
                    samplesPath = s.Substring(5);//s[5..];
                }
                else if (s == "-ask:yes")
                {
                    // no action
                ...
                {
                    Console.WriteLine("SaxonHE12Examples -dir:samples -test:testname -ask:yes|no");
                }
                else
                {
    ...
    public class XsltUsingSourceResolver : Example
    {
        ...
        public override void run(Uri samplesDir)
        {
            ...
            XsltCompiler compiler = processor.newXsltCompiler();
            UserXmlResolver compileTimeResolver = new UserXmlResolver() { Message = "** Calling compile-time XmlResolver: " };
            ...
            UserXmlResolver runTimeResolver = new UserXmlResolver() { Message = "** Calling transformation-time XmlResolver: " };
    ...
    public class XsltUsingResultHandler : Example
    {
        ...
        public override void run(Uri samplesDir)
        {
            ...
            Dictionary<string, XdmDestination> results = new Dictionary<string, XdmDestination>();
            ...
            NullDestination destination = new NullDestination();// { BaseUri = samplesDir };
    ...
    internal class DefaultNamespaceCall : ExtensionFunctionCall
    {
        ...
        public override Sequence call(XPathContext xpc, Sequence[] sarr)
        {
            ...
            {
                return new StringValue(defaultNamespace);
            }
            else
            {
                return EmptySequence.getInstance();
    ...
    public class XsltSimpleExtension : Example
    {
        ...
        private class SqrtFunction : ExtensionFunction
        {
            public XdmValue call(XdmValue[] arg)
            {
                ...
                {
                    return new XdmAtomicValue(Math.Sqrt(((XdmAtomicValue)arg[0]).getDoubleValue()));
                }
            }
